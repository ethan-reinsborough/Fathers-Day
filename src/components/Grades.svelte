<script lang="ts">
  ...
  let { r, old = false }: { r: Regulated; old?: boolean } = $props();
...
<section class="glass card">
  <span class="eyebrow"
    >Every grade · {old ? "saved" : "current"} ceiling · ¢/L</span
  >
  <div class="grid">
    {#each grades as g, i}
      <div class="chip" style="--i:{i}">
        <span class="lab"
          >{g.label}{#if g.hint}<small>{g.hint}</small>{/if}</span
        >
